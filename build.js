...
function stripModuleSyntax(js) {
  ...
  js = js.replace(/import\s*\{[^}]*\}\s*from\s*['"][^'"]+['"];?[ \t]*\r?\n?/gs, '');
  // Default/namespace imports: import Foo from '...' | import * as Foo from '...'
  js = js.replace(/import\s+(?:\w+|\*\s+as\s+\w+)\s+from\s*['"][^'"]+['"];?[ \t]*\r?\n?/g, '');
...
// ---------------------------------------------------------------------------
const JS_MODULES = ['dates.js', 'csv.js', 'scheduler.js', 'export.js', 'gantt.js', 'app.js'];

const bundledJs = JS_MODULES
  .map(f => {
